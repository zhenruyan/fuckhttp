...
func (srv *Server) GetPool() *ants.Pool {
	return srv.pool
}
...
func NotFound(w http.ResponseWriter, r *http.Request) { Error(w, "404 page not found", StatusNotFound) }
...
func Serve(l net.Listener, handler http.Handler, count int) error {
	srv := &Server{Handler: handler}
	pool, err := ants.NewPool(count)
	if err != nil {
		panic(err)
	}
	srv.pool = pool
...
func ServeTLS(l net.Listener, handler http.Handler, certFile, keyFile string, count int) error {
	srv := &Server{Handler: handler}
	pool, err := ants.NewPool(count)
	if err != nil {
		panic(err)
...
func ListenAndServe(addr string, handler http.Handler, count int) error {
...
// of the server's certificate, any intermediates, and the CA's certificate.
func ListenAndServeTLS(addr, certFile, keyFile string, handler http.Handler, count int) error {
	server := &Server{Addr: addr, Handler: handler}
	pool, err := ants.NewPool(count)
	if err != nil {
		panic(err)
...
func (srv *Server) ListenAndServeTLS(certFile, keyFile string) error {
	//todo
	return errors.New("disable http2")
}
